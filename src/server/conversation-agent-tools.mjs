...
export const createConversationAgentTools = ({ appRoot, workspacePath, workspaceKind = "project", requestId, conversationId, sourceMessageId, instruction, taskRoute = null, catalog = [], routeBundle = null, mediaProfiles = {}, mediaDispatch = null, contentOnly = false, readSkill, ask, candidates, media, mediaStatus, browser, signal, emit = () => {}, load = loadWorkspaceState, save = saveWorkspaceState, write = executeDocumentTransaction } = {}) => {
  ...
  const call = async (namespace, name, args) => {
    ...
    if (namespace === "interaction") {
      if (name === "delivery") {
        ...
        if (requestedRoutingMode === "general" && branchRouteRead && !capabilityInspectionOnly) throw new Error("本轮已经读取面板能力分支，必须真实读取对应 Skill 后以 skills 方式交付；不能改报为通用问答");
        delivery = {
          mode: args.mode,
    ...
    if (name === "write") {
      ...
      const taskIdentity = [text(conversationId).trim(), text(sourceMessageId).trim()].filter(Boolean).join(":");
      const fingerprint = digest(args), operationId = `task-${digest(taskIdentity || requestId).slice(0, 20)}-${digest(args.operationId).slice(0, 20)}`;
      ...
      const result = await write({ appRoot, workspacePath, requestId, batchId: `batch-${digest(`${taskIdentity}:${args.operationId}`).slice(0, 32)}`, expectedRevisions, operations: [{ operationId, type: args.operation, targetDocumentId: id, requestedTitle: args.title, targetDirectoryId: moduleId, viewId: args.viewId, folderId: args.folderId, folderLabel: args.folderLabel, parentFolderId: args.parentFolderId, treeGroup: args.treeGroup, formatContractId: args.formatContractId, formatContractVersion: args.formatContractVersion, formatContract: args.formatContract, content, patches: args.patches }], task: { executionSurface: "agent", instruction, sourceMessageId, taskId: sourceMessageId, conversationId, transactionKey: taskIdentity, authorizedCandidate, writeAuthorization: authorization, source: {}, target: {} } });
  ...
  return { protocolVersion: "shensi_conversation_agent_v1", dynamicTools,
    deliveryStatus: () => {
      ...
      const branchRouteRead = readRouteIds.size > 0;
      const declaredGeneral = delivery?.routingMode === "general"
        && Boolean(delivery.routingReason)
        && (!branchRouteRead || capabilityInspectionOnly);
