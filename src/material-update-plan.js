...
export const isMaterialUpdateDocumentId = (documentId = "") => {
  ...
  return /^(?:canon-(?:characters|relations|world|locations|factions|events|items|glossary)|script-canon-(?:characters|relations|world|locations|factions|events|items|glossary)|outline-(?:series|volume-\d+|chapter-\d+)|script-outline-(?:series|episode-\d+)|memory-snapshot|script-memory-snapshot)$/u.test(id);
};
...
export const parseMaterialUpdatePlan = (value = "", {
  ...
  const changes = (Array.isArray(parsed.changes) ? parsed.changes : [])
    .map((item) => {
      ...
      if (/^(?:memory-snapshot|script-memory-snapshot)$/u.test(targetDocumentId)) changeType = "snapshot";
      return {
        targetDocumentId,
...
export const materialUpdateInspectionInstruction = ({ sourceDocuments = [], targetDocuments = [] } = {}) => [
  ...
  "逐条比较正文证据与现有正史设定、相关大纲和状态快照；没有真实差异时 changes 必须为空。不得把语言润色、推测、未来可能性或重复信息当成资料变化。",
  "变化类型只能使用：insert（新增记录）、patch（修改已有记录）、snapshot（当前状态变化）、append（适合末尾累积）、replace（明确推翻整个目标）、create（目标文档不存在且首次写入）。",
