import { agentEngineForProfile, isShensiAgentCompatibleProfile } from "./agent-engine-registry.js";
...
export const runtimeContractForProfile = ({ profile = null, surface = "chat" } = {}) => {
  ...
  if (requestedSurface === "agent"
    && (engine === "codex" || (engine === "codex_api" && text(profile.protocol) === "responses"))
    && model.includes("/")) {
  ...
  if (requestedSurface === "agent" && engine === "codex_api" && !isShensiAgentCompatibleProfile(profile)) {
    return failed({ ...base, code: "CODEX_PROVIDER_MISMATCH", message: "神思运行器只能使用 OpenAI 或自定义兼容接口" });
  }
  if (requestedSurface === "agent" && engine === "codex_api") {
    ...
    if (!["responses", "chat_completions"].includes(text(profile.protocol))) return failed({ ...base, code: "CODEX_API_PROTOCOL_REQUIRED", message: "神思运行器需要 Responses 或已核验的 Chat Completions Agent 协议" });
    if (!text(profile.baseUrl)) return failed({ ...base, code: "CODEX_API_BASE_URL_REQUIRED", message: "神思运行器缺少 API 地址" });
    if (!text(profile.apiKey) && text(profile.credentialSource) !== "public") return failed({ ...base, code: "CODEX_API_KEY_REQUIRED", message: "神思运行器缺少 API Key" });
  }
