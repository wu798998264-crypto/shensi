...
const READ_ONLY_CAPABILITY_QUERY_PATTERN = /(?:(?:本次|这次|此次|当前|刚才).{0,18}(?:实际)?(?:读取|参考|使用|调用|写入|落盘|覆盖)(?:了|过|的是|到)?\s*(?:什么|哪些|哪里|哪(?:个|些)?))|(?:(?:能否|是否|可不可以|会不会|有没有|为什么|为何|怎么|如何|什么原因).{0,36}(?:读取|参考|调用|使用|写入|落盘|覆盖|修改文档|自动保存))|(?:(?:读取|参考|调用|使用|写入|落盘|覆盖|自动保存).{0,28}(?:吗|呢|[?？]|为什么|怎么|如何|哪里|哪些|什么))/iu;
...
const taskAction = ({ text, route, writeAuthorization = null, taskContract = null }) => {
  ...
  if (READ_ONLY_CAPABILITY_QUERY_PATTERN.test(text)) return "analyze";
