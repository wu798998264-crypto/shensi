...
try {
  const result = await runBundledConversationAgent({ appRoot: process.cwd(), machineRoot: root, sessionId: "native-smoke", settings: { protocol: "chat_completions", baseUrl: "https://mock.invalid/v1", model: "poolside/laguna-s-2.1:free", provider: "免费模型" },
    ...
    fetchImpl: async (url, options) => {
      ...
      assert.deepEqual(request.tools.map((tool) => tool.function.name), ["documents_list"]);
