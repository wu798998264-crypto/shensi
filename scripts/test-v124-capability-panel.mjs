...
assert.match(app, /data-open-route-history/u, "完整面板必须提供明确的任务路由历史入口");
assert.match(app, /查看此节点相关变更/u, "模组和模块必须提供按节点过滤的路由变更入口");
assert.match(app, /恢复面板与路由/u, "联合历史必须明确同时恢复面板与路由");
assert.match(app, /面板 v\$\{entry\.version\} · 路由 r/u, "历史列表必须同时显示面板版本和路由版本");
...
assert.match(skillStore, /当前正在使用的任务路由版本不能删除/u);
