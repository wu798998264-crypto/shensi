...
assert.match(app, /await autoPreparePublicTextConnection\(inTaskWorkspace\(\(\) => activeAgentTextProfile\(taskWorkspaceSourceState\.settings\)\), \{ executionSurface: "agent" \}\)\.catch\(\(\) => null\);/u, "Agent 发送链必须在所属作品快照中安全等待准备契约");
