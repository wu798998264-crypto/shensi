...
assert.match(appSource, /taskWorkspaceSourceState\.workspaceKind === "notebook" \|\| routedDeliverableType === "public_account"/u, "作品区发起的公众号文章也必须进入专用公众号笔记本");
assert.match(appSource, /inferredDeliverableType !== "public_account"[\s\S]{0,180}formalTargetCompatibility/u, "公众号文章专用落点不能先被当前作品文档的类型校验截断");
...
assert.match(appSource, /explicitReferenceDocumentIds:[^\n]+recoverableContextGateRetry \? \[\]/u, "用户确认继续后不得循环提交失效引用");
assert.match(appSource, /explicitReferenceDocumentIds: recoverableContextGateRetry[\s\S]{0,100}\? \[\]/u, "Agent 确认继续后也不得循环提交失效引用");
...
assert.match(appSource, /agentRoutedDeliverableType === "public_account"[\s\S]{0,800}notebookDestinationForDeliverable/u, "Agent 公众号成品也必须进入专用笔记本落点");
