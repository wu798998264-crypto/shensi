...
assert.equal(preservedOpenAiSelection.imageConnections.find((profile) => profile.id === "image-user-openai")?.model, "gpt-image-2", "修改默认值不得覆盖用户已有的 OpenAI 图片模型选择");
...
const migratedBuiltInDefaults = normalizeGenerationProfiles({
  ...
  imageConnections: [
    { id: "image-default", name: "OpenAI GPT 图片 CLI", adapter: "cli", provider: "OpenAI", protocol: "images", model: "gpt-image-2", cliPath: "shensi-openai-image" },
    { id: profileId, name: "聚合api", adapter: "api", provider: "自定义兼容接口", protocol: "images", baseUrl: expectedBaseUrl, model: "gpt-image-2" },
  ],
