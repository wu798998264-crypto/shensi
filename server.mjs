...
import { launchMediaGenerationWorker } from "./src/server/media-worker-manager.mjs";
...
import { canonicalMediaProfileSignature } from "./src/server/media-profile-signature.mjs";
...
const conversationAgentApi = createConversationAgentApi({
  appRoot: root,
  gateway: conversationAgentGateway,
  readJsonBody,
  resolveWorkspaceRoot,
  sendJson,
  requestError,
});
const mediaGenerationApi = createMediaGenerationApi({
  appRoot: root,
...
const handleApiRequest = async (request, response, pathname) => {
  ...
  if (await mediaGenerationApi({ pathname, request, requestUrl, response })) return;
  ...
  if (await conversationAgentApi({ pathname, request, response })) return;
